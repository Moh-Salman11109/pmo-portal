import { deriveBudgetStatus } from "./metrics.js";
...
export const exportExcel = (rows, filename, deptMap = {}) => {
  ...
  const HEADERS = ["Code","Project Name","Department","PM","Sponsor","Phase","Status","Progress","Risk","Estimated Cost (SAR)","Actual Cost (SAR)","Cost Status","Gate","Start Date","Planned End"];
  ...
  const dataRows = rows.map((p, i) => {
    ...
    const costStatus = deriveBudgetStatus(p);
    const budgetOk = costStatus !== "Over Estimate";
    return `<tr>
      ...
      ${td(costStatus, b + `color:${budgetOk?"#15803d":"#991b1b"};font-weight:700;`)}
      ${td(p.gate,        b)}
      ${td(p.startDate||"—", b)}
      ${td(p.plannedEnd||"—", b)}
    </tr>`;
